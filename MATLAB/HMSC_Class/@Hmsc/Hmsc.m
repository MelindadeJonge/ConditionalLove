classdef Hmsc < handle
	...
	methods
		...
		sampleMCMC(m, nRuns, append, lastRun, startPar, verb)
		...
		[correlations,combinations] = computeCorrelationsFullP(m,level,Xr)
		R2 = computeR2(m,predN)
		[fixed, fixedsplit, random, traitR2] = computeVariances(m,group)
		[fixed, fixedsplit, random, traitR2] = computeVariancesGradient(m,group,XrVal)
	...
	methods (Static = true)
		...
		eta = update_eta(z,X,Xr,Xs,Xv,beta,sigma,eta,alpha,lambda,etas,lambdas,betav,qv,nf,pi,ncr,spatial,factorCov,iWg,speciesX,includeXs,includeXv)
		alpha = update_alpha(eta,alpha,nf,pi,spatial,iWg,detWg,alphapw)
		z = update_z(z, X,Xr,Xs,Xv,beta,eta,lambda,etas,lambdas,betav,qv,Y,nf,pi,ncr,dist,sigma,speciesX,includeXs,includeXv,factorCov)
		sigma = update_sigma(X,Xr,Xs,Xv,sigma,beta,eta,lambda,etas,lambdas,betav,qv,nf,pi,ncr,dist,asigma,bsigma,z,spatial,factorCov,speciesX,includeXs,includeXv)
