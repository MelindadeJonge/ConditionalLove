function eta = update_eta(z,X,Xr,Xs,Xv,beta,sigma,eta,alpha,lambda,etas,lambdas,betav,qv,nf,pi,ncr,spatial,factorcov,iWg,speciesX,includeXs,includeXv)
[ny ns] = size(z);
isigma = inv(sigma);
if length(spatial)==0
   np = [];
   ...
   if factorcov(r1)
      ...
      if ~spatial(r1)
         ...
         if length(unLpi)==ny % i1 corresponds to residuals, use a faster updater
            for q = 1:ny % change to parfor for time demanding works
               p = lpi(q);
